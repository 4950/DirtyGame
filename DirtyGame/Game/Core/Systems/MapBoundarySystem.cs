    ...
    class MapBoundarySystem : EntitySystem
    {
        ...
        public override void ProcessEntities(IEnumerable<Entity> entities, float dt)
        {
            foreach (Entity e in entities)
            {
                ...
                if (pos.X > renderer.ActiveMap.getPixelWidth() - e.GetComponent<Sprite>().Texture.Width)
                    pos.X = renderer.ActiveMap.getPixelWidth() - e.GetComponent<Sprite>().Texture.Width;
                if (pos.Y > renderer.ActiveMap.getPixelHeight() - e.GetComponent<Sprite>().Texture.Height)
                    pos.Y = renderer.ActiveMap.getPixelHeight() - e.GetComponent<Sprite>().Texture.Height;

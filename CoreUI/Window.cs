...
namespace CoreUI
{
    public class Window : ContentElement
    {
        ...
        protected internal override void OnSizeChanged(object sender)
        {
            ...
            if (rs != null)
                rs.resize((int)Bounds.Width, (int)Bounds.Height);
            else
            {
                rs = CoreUIEngine.mDrawEngine.CreateRenderSurface((int)Bounds.Width, (int)Bounds.Height);
            }
            mWindowChrome.Resize();
        }
        public void Show()
        ...
        protected internal void TexRender()
        {
            if (State != WindowState.Minimized)
            {
                if (Flashing)
                {
                    FlashTick += CoreUIEngine.TimeElapsed;
                    if (FlashTick > 500)
                    {
                        FlashTick = 0;
        ...
        protected internal override void Render()
        {
            if (Invalidated)
            {
                CoreUIEngine.mDrawEngine.BeginDraw(rs);
                //rs.StartRender();
                //CoreUIEngine.mDrawEngine.Action_Begin2D();
                CoreUIEngine.mDrawEngine.Draw_FilledBox(0, 0, Bounds.Width, Bounds.Height, Background);
                base.Render();
                //CoreUIEngine.mDrawEngine.Action_End2D();
                //rs.EndRender();
                CoreUIEngine.mDrawEngine.EndDraw();
            }
            if (Flashing)
                InvalidateVisual();
        }

            ...
            if ((wc.Ammo > 0 || wc.MaxAmmo == -1) && wc.LastFire <= 0)
            {
                ...
                else if (wc.Type == WeaponComponent.WeaponType.Ranged)
                {
                    ...
                    else if (wc.WeaponName == "GrenadeLauncher")
                    {
                        ...
                        Entity grenade = game.entityFactory.CreateGrenade(Owner, spatial.Center, dir, wc.ProjectileSprite, (float)dist, wc.ProjectileSpeed, .5f, new Vector2(128, 128), Weapon);
                        grenade.Refresh();
                    }
